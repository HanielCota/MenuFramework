...
/**
 * Folia scheduling for one player: re-renders run on that player's region thread via their
 * {@code EntityScheduler}, the only thread allowed to touch their inventory.
 *
 * <p>If the player has logged out (or their region unloaded) the task is dropped — there is
 * nothing to re-render and no inventory to touch. The view is torn down separately on close.
 */
